...
import ar.edu.itba.paw.exceptions.BuyOrderIsNotPendingException;
...
@Provider
public class BuyOrderIsNotPendingExceptionMapper implements ExceptionMapper<BuyOrderIsNotPendingException> {

    @Autowired
    private MessageSource messageSource;

    @Override
    public Response toResponse(BuyOrderIsNotPendingException e) {
        final String errorMessage = messageSource.getMessage(e.getMessage(), null, LocaleContextHolder.getLocale());
        final ErrorDto error = ErrorDto.fromGenericException(e, 400, errorMessage, ApiReturnCodes.BUYORDER_IS_NOT_PENDING.getCode());
