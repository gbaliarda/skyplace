    ...
    <div className=" border-gray-200 rounded-2xl border flex-col justify-between mb-8 bg-slate-50">
      ...
      <div className="flex items-center justify-between py-3 px-4">
        <div suppressHydrationWarning className="font-semibold flex items-center">
          <img className="w-10 h-10 rounded-full mr-3" src="/profile/profile_picture.png" alt="" />
          {t("product.offerBy").toLowerCase()}
          {loading ? (
            <Spinner />
          ) : (
            <Link href={`/profile/${user!!.id}`}>
              <span className="ml-1 text-cyan-600 hover:underline hover:text-cyan-800 cursor-pointer">
                {user!!.username}
              </span>
            </Link>
          )}
        ...
        <div className="flex items-center">
          ...
          <span className="text-slate-700">{pendingBuyOrder.amount}</span>
        </div>
      </div>
    </div>
  )
